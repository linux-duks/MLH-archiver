...
use mlh_parser2::email_parser::parse_email;
use std::fs;
...
fn test_synthetic_date_fallback() {
    ...
    for email_file in &email_files {
        ...
        let expected_date = mlh_parser2::date_parser::parse_date_tentative_raw(&expected_date_str);

        let result = parse_email(&mail_bytes, now).unwrap();

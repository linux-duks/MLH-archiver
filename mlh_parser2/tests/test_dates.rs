...
use mlh_parser2::date_parser::{parse_date_tentative_raw, process_date};
use mlh_parser2::email_reader::{decode_mail, get_headers};
use std::fs;
...
fn test_millennium_dates() {
    ...
    for (found_str, expected_str) in millennium_cases {
        ...
        let fixed = mlh_parser2::date_parser::fix_millennium_date(found_date, now);
